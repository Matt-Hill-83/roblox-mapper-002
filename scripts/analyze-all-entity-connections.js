...
function extractRelationGUIDs(filePath) {
  try {
    const content = fs.readFileSync(filePath, 'utf8');
    const sourceGuidMatches = content.match(/"source_guid":\s*"([^"]+)"/g) || [];
    const targetGuidMatches = content.match(/"target_guid":\s*"([^"]+)"/g) || [];
    
    const sourceGuids = sourceGuidMatches.map(match => match.match(/"([^"]+)"/)[1]);
    const targetGuids = targetGuidMatches.map(match => match.match(/"([^"]+)"/)[1]);
...
function analyzeAllEntityConnections() {
  ...
  console.log('🔍 Analyzing ALL entity types for relationship connections...\n');
  
  // Get all entity files
  const entityFiles = fs.readdirSync(dataDir)
    .filter(file => file.startsWith('entity') && file.endsWith('Data.ts'))
    .map(file => path.join(dataDir, file));
  
  // Get all relation files and collect all relation GUIDs
  const relationFiles = fs.readdirSync(dataDir)
  ...
  console.log(`📊 Total unique GUIDs in all relation files: ${allRelationGUIDs.size}\n`);
  ...
  entityFiles.forEach(file => {
    ...
    const connectedGUIDs = entityGUIDs.filter(guid => allRelationGUIDs.has(guid));
    const connectionRate = entityGUIDs.length > 0 ? (connectedGUIDs.length / entityGUIDs.length) * 100 : 0;
  ...
  console.log('Entity Type'.padEnd(25) + 'Total'.padEnd(8) + 'Connected'.padEnd(12) + 'Rate'.padEnd(8) + 'Status');
  console.log('─'.repeat(70));
  
  results.forEach(result => {
    const status = result.connectionRate === 0 ? '❌ Isolated' : 
                   result.connectionRate < 50 ? '🔶 Sparse' : '✅ Well connected';
  ...
  console.log(`❌ Entity types without connections: ${results.length - wellConnected.length}`);
  
  if (wellConnected.length > 0) {
    console.log(`\n🏆 TOP CONNECTED ENTITY TYPES (for visualization):`);
    wellConnected.slice(0, 5).forEach((result, index) => {
      console.log(`  ${index + 1}. ${result.entityType}: ${result.connectedCount}/${result.totalCount} (${result.connectionRate.toFixed(1)}%)`);
    });
    
    console.log(`\n💡 RECOMMENDATION:`);
    console.log(`   Focus your entity ring on the top connected entity types for maximum connections.`);
    console.log(`   Consider using only: ${wellConnected.slice(0, 8).map(r => r.entityType).join(', ')}`);
  }
}
