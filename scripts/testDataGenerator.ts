...
// Import the types and generator
import { GeneratorConfig, Cluster, Node, Link } from '../src/shared/interfaces/simpleDataGenerator.interface';

// Test configuration object
const TEST_CONFIG: GeneratorConfig = {
  numLevel1Nodes: 2,      // 2 root nodes
...
// Mock the generator constants and service locally for testing
const GENERATOR_CONSTANTS = {
  ...
  PEOPLE_NAMES: ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Henry", "Iris", "Jack"],
  ANIMAL_NAMES: ["Fifi", "Bongo", "Rex", "Luna", "Max", "Bella", "Rocky", "Daisy", "Shadow", "Buddy"],
  ANIMAL_TYPES: ["cat", "dog", "bird", "rabbit", "hamster", "fish", "turtle", "snake", "lizard", "parrot"],
...
class MockDataGenerator {
  ...
  generateCluster(config: GeneratorConfig): Cluster {
  ...
  private createNode(level: number): Node {
    const types: Array<"People" | "Animals"> = ["People", "Animals"];
    ...
    if (type === "People") {
      ...
      node.properties!.animalType = GENERATOR_CONSTANTS.ANIMAL_TYPES[
  ...
  private getRandomName(type: "People" | "Animals"): string {
    const names = type === "People" ? GENERATOR_CONSTANTS.PEOPLE_NAMES : GENERATOR_CONSTANTS.ANIMAL_NAMES;
    return names[Math.floor(Math.random() * names.length)];
  }
  
  private createHierarchy(nodesByLevel: Map<number, Node[]>, maxChildren: number, cluster: Cluster): void {
  ...
  private connectLevels(parents: Node[], children: Node[], maxChildren: number, cluster: Cluster): void {
  ...
  private createRelations(nodes: Node[], numLinkTypes: number, cluster: Cluster): void {
    const linkTypes = ["Owns", "Wants", "Eats", "Link4", "Link5"].slice(0, numLinkTypes);
    ...
    for (let i = 0; i < maxLinks; i++) {
      ...
      cluster.relations.push({
        ...
        color: GENERATOR_CONSTANTS.LINK_COLORS[linkType as keyof typeof GENERATOR_CONSTANTS.LINK_COLORS] || [0.5, 0.5, 0.5]
      });
    }
  }
}
...
function generateMermaidDiagram(cluster: Cluster): string {
  ...
  cluster.groups.forEach(group => {
    const peopleNodes = group.nodes.filter(n => n.type === "People").map(n => n.uuid).join(',');
    const animalNodes = group.nodes.filter(n => n.type === "Animals").map(n => n.uuid).join(',');
...
function generateSummary(cluster: Cluster, config: GeneratorConfig): string {
  ...
  summary += `- Total Nodes: ${cluster.groups.reduce((sum, g) => sum + g.nodes.length, 0)}\n`;
...
function main() {
  ...
  const outputFile = path.join(outputDir, `data-generator-test-${timestamp}.md`);
  ...
  console.log('\nGeneration Summary:');
  console.log(`- Total Nodes: ${cluster.groups.reduce((sum, g) => sum + g.nodes.length, 0)}`);
