...
41. highlight links on node click.
  1. when user clicks on a node, all links connected to the node change to the LINK_HILIGHT_COLORS[0]
  2. define LINK_HILIGHT_COLOR in the existing robloxColors.ts
  3. when they click on another node, the links of that node change to LINK_HILIGHT_COLORS[0]
  4. Any common links between the 2 link sets get a third color, that is a hue in between.  ex: blue, yellow --> green