...
## New Items
1. ✅ Change space between y levels to 5 (already set to 5 in RENDERER_CONSTANTS.POSITIONING.LEVEL_SPACING)
2. ✅ Figure out why links are not working (fixed - node UUID mismatch with harness_file_ vs harness_node_)
3. ⬛ Clean unused items from GUI
   - ⬛ Remove num node types
   - ⬛ Remove num link types
   - ⬛ Remove num pet types
4. ✅ Wrap all Y shadows in a Model (completed - YParallelShadows model)
5. ✅ In the dropdowns for the axis properties, put the number of types of that property like this: service [15]
6. ⬛ Refactor dropdowns into grid
7. ⬛ Nix duplicate walls
8. ⬛ Push Y shadows to edge
9. ⬛ Option for stacks on Y axis
10. ⬛ Stacks and Y axis
11. ⬛ Pass in default axis options from game.service