...
export class HierarchicalRopeConnector {
  ...
  public createHierarchicalConnections(
    ...
    print(
      `[HierarchicalRopeConnector] Created ${allConnections.size()} hierarchical connections`
    );
  ...
  public createHierarchicalRope(
    ...
    if (!fromEntity || !toEntity) {
      print(
        `⚠️ Cannot create rope: missing entities ${connection.fromGuid} -> ${connection.toGuid}`
      );
      return undefined;
    }
    ...
    try {
      const ropeInstance = createRopeLabel({
        ...
        parent: parentFolder,
        props: {
          relationName: `${connection.fromGuid}_${connection.relationshipType}_${connection.toGuid}`,
        },
      });
      ...
      if (ropeInstance) {
        ...
        print(
          `✅ Created hierarchical rope: ${connection.fromGuid} -> ${connection.toGuid}`
        );
      }
  ...
  private createParentChildRope(
    ...
    try {
      const ropeInstance = createRopeLabel({
        ...
        parent: parentFolder,
        props: {
          relationName: `${connection.fromGuid}_owns_${connection.toGuid}`,
        },
      });
      if (ropeInstance) {
        ropeInstance.Parent = parentFolder;
        ...
        if (attachment1 && attachment2) {
          try {
            const ropeInstance = createRopeLabel({
              ...
              parent: parentFolder,
              props: {
                relationName: `${entity1.guid}_level${level}_${entity2.guid}`,
              },
            });
            if (ropeInstance) {
              ropeInstance.Parent = parentFolder;
