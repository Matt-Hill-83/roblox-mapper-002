import { TREE_SPACING, LEVEL_HEIGHT, ENTITY_SPACING, BASE_HEIGHT } from './layoutConstants';
...
export class DynamicSpacingCalculator {
    ...
    public calculateDynamicSpacing(trees: TreeLayout[]): DynamicSpacingConfig {
        print("[DynamicSpacingCalculator] Calculating dynamic spacing for", trees.size(), "trees");
        ...
        print("[DynamicSpacingCalculator] Dynamic spacing calculated:", spacingConfig);
    ...
    public calculateTreeSpacing(
        ...
        const complexityFactor = math.sqrt(tree1.totalEntities + tree2.totalEntities) / 10;
        
        // Calculate dynamic spacing
        const dynamicSpacing = baseSpacing + (tree1Width + tree2Width) / 2 + complexityFactor;
    ...
    private analyzeTreeCharacteristics(trees: TreeLayout[]): TreeCharacteristics[] {
    ...
    private calculateOptimalSpacing(characteristics: TreeCharacteristics[]): DynamicSpacingConfig {
        if (characteristics.size() === 0) {
            return this.getDefaultSpacingConfig();
        }
        
        // Calculate average characteristics
        const avgDepth = characteristics.reduce((sum, char) => sum + char.depth, 0) / characteristics.size();
        const avgEntityCount = characteristics.reduce((sum, char) => sum + char.entityCount, 0) / characteristics.size();
    ...
    private calculateBoundingBoxVolume(boundingBox: { min: [number, number, number]; max: [number, number, number] }): number {
    ...
    private groupEntitiesByLevel(entityPositions: EntityPosition[]): Map<number, EntityPosition[]> {
