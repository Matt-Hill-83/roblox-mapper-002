...
import {
  ...
  HierarchyTree,
  HierarchyNode,
} from "./hierarchicalLayoutCalculator";
...
export class HierarchyLayoutIntegrator {
  ...
  public convertToLayoutFormat(): HierarchyAnalysisResult {
    print(
      "[HierarchyLayoutIntegrator] Converting hierarchy analysis to layout format"
    );
    ...
    print(
      "[HierarchyLayoutIntegrator] Converted",
      result.trees.size(),
      "trees for layout"
    );
    return result;
  ...
  public getEnhancedHierarchyAnalysis(): HierarchyAnalysisResult {
    ...
    const enhancedTrees = baseAnalysis.trees.map((tree) =>
      this.enhanceTreeMetadata(tree)
    );
   ...
   */
  public getFilteredHierarchyForLayout(
    excludeTypes: string[] = []
  ): HierarchyAnalysisResult {
    const fullAnalysis = this.convertToLayoutFormat();

    // Filter out unwanted entity types
    const filteredTrees = fullAnalysis.trees.map((tree) =>
      this.filterTree(tree, excludeTypes)
    );
...
export function getFilteredHierarchyForLayout(
  excludeTypes: string[] = ["artifact", "flag"]
): HierarchyAnalysisResult {
  const integrator = new HierarchyLayoutIntegrator();
