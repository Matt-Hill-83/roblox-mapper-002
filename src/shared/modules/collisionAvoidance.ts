...
export class CollisionAvoidanceSystem {
    ...
    if (preserveHierarchy) {
      ...
      levelGroups.forEach((entities, level) => {
        ...
        for (const adjustedEntity of levelAdjusted) {
          const index = adjustedPositions.findIndex(
            (e) => e.guid === adjustedEntity.guid
          );
          if (index !== -1) {
  ...
  public createCollisionFreeZones(
    ...
    for (const importantGuid of importantEntityGuids) {
      const importantEntity = adjustedPositions.find(
        (e) => e.guid === importantGuid
      );
  ...
  private resolveCollisionsInLevel(
  ...
  private calculateRepulsionForce(
    ...
    return [direction[0] * forceMultiplier, 0, direction[2] * forceMultiplier];
  }
}
