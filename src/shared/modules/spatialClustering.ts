...
export class SpatialClustering {
    ...
    public applyClustering(entityPositions: EntityPosition[]): EntityPosition[] {
        ...
        entityGroups.forEach((entities, entityType) => {
            const clusteredGroup = this.applyTypeBasedClustering(entities, entityType);
     ...
     */
    public calculateClusterCentroid(entities: EntityPosition[]): [number, number, number] {
        if (entities.size() === 0) {
            return [0, 0, 0];
        }
    ...
    public adjustClusterSpacing(
        ...
        for (let i = 0; i < adjustedPositions.size(); i++) {
            ...
            const originalEntity = hierarchicalPositions.find(e => e.guid === clusteredEntity.guid);
            ...
            if (originalEntity) {
                ...
                adjustedPositions[i] = {
                    ...clusteredEntity,
                    position: [
                        originalEntity.position[0] * (1 - blendFactor) + clusteredEntity.position[0] * blendFactor,
                        originalEntity.position[1], // Keep hierarchical Y position
                        originalEntity.position[2] * (1 - blendFactor) + clusteredEntity.position[2] * blendFactor
                    ]
                };
            }
        }
     ...
     */
    public createTypeSpecificClusters(entities: EntityPosition[]): Map<string, EntityPosition[]> {
        ...
        for (const entity of entities) {
            const strategy = clusteringStrategies.get(entity.clusterGroup) || "circular";
    ...
    private groupEntitiesByType(entityPositions: EntityPosition[]): Map<string, EntityPosition[]> {
    ...
    private applyTypeBasedClustering(entities: EntityPosition[], entityType: string): EntityPosition[] {
        if (entities.size() <= 1) {
            return entities;
        }
