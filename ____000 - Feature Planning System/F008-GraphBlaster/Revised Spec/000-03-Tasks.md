...
1. ⬛ T18.1: Update Data Generation for Persons
   - Modify data generator to create exactly 50 persons
   - Each person has 3 key properties: petType, countryOfResidence, favoriteColor
   - Each property should have exactly 5 possible values

2. ⬛ T18.2: Define Property Types and Values
   - petType: ["Dog", "Cat", "Bird", "Fish", "Hamster"]
   - countryOfResidence: ["USA", "Canada", "Mexico", "Brazil", "UK"]
   - favoriteColor: ["Red", "Blue", "Green", "Yellow", "Purple"]

3. ⬛ T18.3: Create Axis Mapping Configuration
   - X axis = petType (5 positions)
   - Y axis = countryOfResidence (5 positions)  
   - Z axis = favoriteColor (5 positions)
   - Total grid: 5x5x5 = 125 possible positions

4. ⬛ T18.4: Implement Coordinate Calculator
   - Function to map person properties to 3D grid position
   - Input: person object with 3 properties
   - Output: Vector3 position in rubix cube

5. ⬛ T18.5: Create Node Renderer
   - Place 1x1x1 blocks at calculated positions
   - Blocks should be inside the rubix cube grid
   - Handle multiple persons at same position
...
12. ⬛ T18.12: Clean Up Debug Statements
    - Remove debug prints from wireframeBlockMaker
    - Clean up other temporary debug code 